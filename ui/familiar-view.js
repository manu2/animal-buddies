...
export function familiarView({mode,animal,choices,animals,correct,wrong,round,limited}){
 ...
 (correct?'<button class="primary" id="next"><span class="big-arrow" aria-hidden="true">→</span><span class="play-label">'+(limited&&round===5?'Say goodbye':'Next friend')+'</span></button>':'')+
 (limited?'<div class="steps familiar-progress" aria-label="Turn '+(round+1)+' of 6">'+dots+'</div>':'')+'</section>';
}
