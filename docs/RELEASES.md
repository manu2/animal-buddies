...
## Illustrated familiar play and physical routines — 2026-09-21, worker v12 / schema 3
Candidate scope D19, preserving D01–D18: consistent portraits for all eight animals; direct names/sentence/letter choices and quiet outcomes; saved picture order; Cow/Rabbit held-brush and seated table poses; reusable bathroom/dining rooms. Original action sheets, lake classroom, voice recordings, three levels, daily budget and checkpoints preserved. Bathing is not implemented.

Failures found before publication: initial familiar suite failed because global step easing skipped intermediate pose frames (I28). Fixed per-keyframe timing in both new routines and original actions; added explicit frame assertions. Visual inspection then found adjacent lion artwork leaking into the wide elephant outcome (I29), fixed with inner atlas-cell clipping. Earlier body-motion tests and school-only visual review were insufficient; neither is now treated as proof of all-level quality.

Verification in progress; no publication claim until final results below. Exact image prompts/assets: ARTWORK_ROUTINES.md. Remaining limitations: two-pose animation, no actual phone installation/human listening/child comprehension or learning-transfer pass.

Local technical gate: PASS, all ten suites (state/content/day/familiar/audit/pilot/daily/language/motion/upgrade); evidence rooms-local-release.json and rooms-local-release.txt. All 233 unchanged voice clips decoded offline; measured pause 1216ms. Actual v11 names pending-outcome upgrade preserves settings, deadline and turn receipt and continues into the new direct choices offline. Historical v4/v10 and current day/school upgrades also pass.

Separate direction/visual review: PASS for this change. Three levels remain equally available, old natural action sheets and approved lake are byte-for-byte unchanged; all eight illustrated portraits inspected in rendered phone outcomes, matching choices reviewed, and both heroes' bathroom and table poses inspected with phone/desktop layouts. Evidence familiar-*.png and rooms-*.png. Inner clipping removes the adjacent lion from Elephant's wide card. Bathroom sink/mirror/tub and dining setting are recognisable; table/chair are part of the meal pose, avoiding a duplicated table. This is assistant review, not child recognition evidence. Exact room/pose limitations remain above.

Public gate: PASS. GitHub Pages reports built app commit fc0df1f606b8d7e855ea1aeb580dfe3067fa0aea. Public familiar/day/language suites passed, including all eight portraits, rooms/poses, complete day and offline decoding of all 233 recordings with 1215ms pause. Evidence rooms-public-familiar.txt, rooms-public-day.txt, rooms-public-language.txt. These are three public suites; all ten ran locally.

In-app browser handoff: no prior tabs remained in the browser inventory, so opened a public tab. Observed the retained completed visit and automatic worker update. Parent settings still showed English, seven minutes, existing Cow morning/school/bedtime practice and earlier school practice counts. Used Restart visit now after the update and left the three-level chooser visible, offline ready, without starting its timer. Marked that tab to remain open. No runtime changes after public verification; this follow-up is documentation and evidence only.
