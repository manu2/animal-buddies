...
D22 (2026-09-21): make home-screen installation discoverable on the chooser, with optional parent-facing help. Use the native install confirmation only when the browser supplies beforeinstallprompt; otherwise show device-specific manual instructions. iPhone installation remains a browser-controlled Share/Add action. Never auto-open a prompt, claim installation from a click alone, interrupt play or change game saves. Hide help in standalone mode; Not now hides the chooser card until reload.
