...
import {DAY,HEROES,validateStory,storyClips,choicesFor} from '../content/day.js';
...
for(const [id,node] of Object.entries(DAY))for(const phase of ['choose','help','outcome'])for(const lang of ['en','hi']){
 ...
 const choices=choicesFor(node,phase);assert.ok(choices.length>=1&&choices.length<=2);assert.equal(new Set(choices.map(c=>c.id)).size,choices.length);
}
for(const id of ['day-levels','day-lobby','day-retry'])for(const lang of ['en','hi'])assert.ok(clips.has(id+'-'+lang));
for(const path of ['./content/day.js','./engine/journey.js','./engine/save-store.js','./engine/day-controller.js','./ui/day-view.js','./day.css','./scenes/animal-home-v1.png','./scenes/lakeside-classroom-v2.png',...HEROES.map(h=>'./stories/'+h+'.png')])assert.ok(assets.has(path),'Not cached '+path);
