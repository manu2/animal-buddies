...
const server=http.createServer((req,res)=>{
 ...
 try{let body=!modern&&old[path]?old[path]:fs.readFileSync(root+'/'+path);if(modern&&path==='sw.js'&&revision)body=Buffer.from(body.toString().replace(/offline-v[0-9]+/, '$&-upgrade-test-'+revision));res.writeHead(200,{'Content-Type':type,'Cache-Control':'no-store'});res.end(body);}catch{res.writeHead(404);res.end();}
});
