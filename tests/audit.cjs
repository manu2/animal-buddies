...
(async()=>{const b=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await p.locator('#day-next').click();assert.equal(await p.locator('.bedroom-scene').getAttribute('data-bedding'),'awake','Not already asleep before choosing');await p.locator('[data-day-choice=sleep]').click();assert.equal(await p.locator('.bedroom-scene').getAttribute('data-bedding'),'asleep');
